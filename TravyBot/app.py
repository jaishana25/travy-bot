...
def fetch_travel_information_for_user_queries(query, travel_type):
    ...
    if travel_type == "General Travel Information":
      prompt_temp =  f"""prompt": "You are tasked with developing a travel planner system that handles multiple questions from the users related to 
      various modes of transportation, accommodations, and activities. The system should act as a virtual travel planner, providing information about 
      distances between cities, travel times, and costs for different modes of transportation (such as flights, trains, buses, or driving). When users 
      inquire about travel options between places, the system should showcase virtual data including distances between cities, travel times, and cost 
      ranges for different transportation options. Additionally, the system should be able to assist users in planning their travel itineraries, 
      suggesting routes, modes of transportation, accommodations, and activities based on their preferences and constraints.",,
        "user_question": "{query}"""
